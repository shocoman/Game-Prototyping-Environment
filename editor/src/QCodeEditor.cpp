    ...
    m_tabReplace(QString(4, ' '))
{
    ...
    setDefaultSyntaxAndHighlighter();
}


void QCodeEditor::wheelEvent(QWheelEvent *event){
    ...
    if (event->modifiers() == Qt::Modifier::CTRL) {
        ...
        auto currentFontSize = font().pointSize();
        if (event->delta() > 0)
            currentFontSize += 4;
        else
            currentFontSize -= 4;
        ...
        fnt.setPointSize(currentFontSize);
...
void QCodeEditor::setDefaultSyntaxAndHighlighter(){
    ...
    m_highlighter = new QLuaHighlighter;
    if (m_highlighter)
    {
...
QString QCodeEditor::getFilePath() const
{
    return filePath;
}

void QCodeEditor::setFilePath(const QString &value)
{
    filePath = value;
}
...
void QCodeEditor::updateLineNumberArea(const QRect& rect)
{
    m_lineNumberArea->update(0, rect.y(),
        m_lineNumberArea->sizeHint().width(), rect.height());
    updateLineGeometry();
...
void QCodeEditor::keyPressEvent(QKeyEvent* e) {
  ...
  if (!completerSkip) {
    ...
    auto lineIsCommented = [](QString line) {
      for (int i = 0; i < line.length()-1; ++i) {
          if (line[i] == '-' && line[i+1] == '-') {
              return i;
          }

          if (!line[i].isSpace()) break;
      }
      return -1;
    };


    // Comment / Uncomment selected lines of code
    if (e->key() == Qt::Key::Key_Slash && e->modifiers() == Qt::Modifier::CTRL){
        auto cursor = textCursor();
        ...
        auto cursorStart = cursor;
        cursorStart.setPosition(selectionStart);
        auto cursorEnd = cursor;
        cursorEnd.setPosition(selectionEnd);

        auto linesToComment = cursorEnd.blockNumber() - cursorStart.blockNumber() + 1;

        cursor.setPosition(selectionStart);
        setTextCursor(cursor);

        bool linesAreCommented = true;
        for (int i = 0; i < linesToComment; i++) {
            cursor.movePosition(QTextCursor::MoveOperation::StartOfLine);

            auto currentLine = cursor.block().text();
            int pos = lineIsCommented(currentLine);
            if (pos == -1){
        ...
        for (int i = 0; i < linesToComment; i++) {
            ...
            if (i == linesToComment-1) continue;
            cursor.movePosition(QTextCursor::MoveOperation::Down);
        }
    }
